...
namespace Noord.HollandsArchief.Pre.Ingest.WorkerService.Handler.Command
{
   public class SipCreateCommand : AbstractPreingestCommand
    {
        public override ValidationActionType ActionTypeName => ValidationActionType.SipCreatorHandler;
        public SipCreateCommand(ILogger<PreingestEventHubHandler> logger, Uri webapi) : base(logger, webapi) { }
        ...
        public override void Execute(HttpClient client, Guid currentFolderSessionId, Settings settings)
        {
            TryExecuteOrCatch(client, currentFolderSessionId, (id) =>
            {
                ...
                api.SipcreatorAsync(id).GetAwaiter().GetResult();
            });
        }
    }
}
